...
## 1.3.0 (2026-09-20)
- **Stop フックは、最後の返答に完了の目印があるときだけ動く**（`scripts/notify_gate.js`）。
  目印は「動作確認できます」「完成です」「【完了】」「【判断待ち】」の 4 つ（返答を書く側が最終報告にだけ入れる・恒久ルール）。
  途中経過・バックグラウンドの待ち・Monitor の起床・サブエージェントやワークツリー単位の区切りで返答が終わっても、
  テストも通知（音＋ポップアップ）も一切出さない（notify.log に「skip: 途中経過」とだけ残す）
- 目印があれば これまでどおり テスト → 通ればここで 1 回だけ通知。音と窓を出す仕掛けはこの 1 本（`notify.ps1`）だけ
- 単体テスト `node scripts/test_notify_gate.js`（試験モードで Stop フック本体を動かし、途中経過は鳴らず【完了】で 1 回だけ鳴ると確かめる）

## 1.2.1 (2026-09-20)
- 環境変数 `DEV_GUARD_QUIET=1` が付いた claude（別のプログラムが `claude -p` で呼ぶもの）では、すべてのフックを素通りする
  （Pocket Summit がAIを呼ぶたびに「pocket-summit-neutral の作業が終わりました」が鳴っていた。呼ぶ側が付ける）

## 1.2.0 (2026-09-20)
- 通知は「指示全体が本当に終わったとき 1 回」だけ（恒久ルール 10）
  - SubagentStop では鳴らさない（数を減らすだけ）。サブエージェントが終わっても本体はその結果を受けて続きをやるため
  - 本体の Stop で裏のエージェントが残っていれば鳴らさない（保留のまま）。本体が再開して最後に止まったとき（残り 0）に 1 回
  - 10 分の保険通知（notify_watcher.js）を廃止（途中で鳴る原因だった）
